            ...
            <div class="nav nav-item  navbar-nav-right ml-auto">
                ...
                @can('الاشعارات')
                    <div class="dropdown nav-item main-header-notification">
                        ...
                        <div class="dropdown-menu">
                            ...
                            <div class="menu-header-content bg-primary text-right">
                                <div class="d-flex">
                                    <h4 class="dropdown-title mb-1  text-white font-weight-semibold">الاشعارات
                                    </h4>
                                    @if (auth()->user()->unreadNotifications()->count() >= 1)
                                        <span class="badge badge-pill badge-warning mr-auto my-auto float-left">
                                            <a href="{{ route('markAllRead') }}"> جعل الاشعارات كلها مقروءه </a>
                                        </span>
                                    @endif
                                </div>
                                @if (auth()->user()->unreadNotifications()->count() >= 1)
                                    <h5 class="dropdown-title-text subtext mb-0 text-white op-6 pb-0">الاشعارات الغير
                                        مقروءه
                                        <span class="tx-danger tx-30"
                                            id="notification_count">{{ auth()->user()->unreadNotifications()->count() }}</span>
                                    </h5>
                                @endif
                            </div>
                            @if (auth()->user()->unreadNotifications()->count() >= 1)
                                <div style="margin-top: 10px;height: 70px;overflow:scroll;"
                                    class="main-notification-list Notification-scroll">
                                    @foreach (auth()->user()->unreadNotifications as $notification)
                                        <div class="mr-3" id="unreadnotification">
                                            @php

                                                $invoices = \App\Models\InvoicesDetailes::where('id', $notification->data['id'])->get();
                                            ...
                                            <a style="color: blue;text-decoration-line: underline"
                                                ...
                                                href="{{ route('invoice-detailes.show', $invoice) }}">

                                                {{ $notification->data['title'] }} :
                                                {{ $notification->data['user'] }}
                                            </a>
                                            <div class="notification-subtext">الوقت :


                                                {{ date_format($notification->created_at, 'h:i:s Y-m-d') }}
                                            </div>
                                            <hr>
                                        </div>
                                    @endforeach
                            ...
                            @else
                                <p style="margin-top:15px;color: red " class="tx-center">لا توجد اشعارات الأن</p>
                            @endif
                        </div>
                    </div>
                @endcan
                ...
                <div class="dropdown main-profile-menu nav nav-item nav-link">
                    <a class="profile-user d-flex" href=""><img alt=""
                            src="{{ URL::asset('assets/img/faces/FB_IMG_1594392977125.jpg') }}"></a>
                    <div class="dropdown-menu">
